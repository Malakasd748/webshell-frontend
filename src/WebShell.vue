<template>
  ...
  >
    <NLayoutHeader style="background-color: var(--panel-background-color)" bordered>
      <TermHeader
    ...
    <NLayout has-sider class="grow chat-drawer-target">
      <NLayoutSider
      ...
      <NLayout class="h-full" content-class="flex-(~ col)">
        <TermSettings v-show="activeTab === 'settings'" class="grow" />
        <TermBody
          v-for="t in terms"
          :key="t.id"
          v-show="activeTab === t.id"
...
<script setup lang="ts">
  ...
  import { NLayout, NLayoutHeader, NLayoutFooter, NLayoutSider, NButton } from 'naive-ui';
