<template>
  <NListItem class="text-12px" ref="itemRoot">
    <NGrid>
      <NGi :span="16" class="flex-(~ gap-2 items-center)">
        <div
          :class="
        ...
        <div class="flex-(~ col) max-w-54">
          <NEllipsis :style="{ color: textColorBase }" :tooltip="{ contentClass: 'w-max' }">
            {{ session.name }}
          </NEllipsis>
      ...
      <NGi :span="4" class="flex-(~ gap-2 items-center)">
        <span
      ...
      <NGi :span="4" class="flex-(~ gap-2 items-center justify-end)">
        <NTooltip v-if="removableStatuses.includes(session.status.value!)" content-class="w-max">
          清除
          <template #trigger>
            <NButton quaternary size="tiny" @click="emit('remove', session)">
              <span class="i-ant-design:delete-outlined"></span>
            </NButton>
          </template>
        </NTooltip>
        <NTooltip v-if="redoableStatuses.includes(session.status.value!)" content-class="w-max">
          重传
          <template #trigger>
            <NButton quaternary size="tiny" @click="emit('redo', session)">
        ...
        >
          取消
          <template #trigger>
            <NButton quaternary size="tiny" @click="emit('cancel', session)">
              <span class="i-ant-design:close-outlined"></span>
            </NButton>
          </template>
        </NTooltip>

        <div class="flex items-center" v-if="session.status.value === 'uploading' && !hovered">
          <img :src="SpinnerImg" class="spinner ml-2 size-14px object-contain" />
        </div>
      </NGi>
    </NGrid>
...
<script setup lang="ts">
  ...
  type UploadListItemMap = {
    [status in SessionStatus]: [string, string];
  };
